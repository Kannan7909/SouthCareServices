...
<head>
    ...
    <title>Health Questionnaire Details</title>
     ...
     <table class="table table-striped table-hover table-bordered">
                ...
                <tbody class="allData">
                    ...
                    <th scope="row">{{ $loop->iteration }}</th>
                        ...
                        <td>
                        @if($employee->health_verified == 'processing')
                             <a href="{{ route('approve.health',encrypt($employee->id)) }}" class="edit btn btn-primary" title="Confirm" data-toggle="tooltip"><i class="material-icons">&#xE254;</i>Appove Health Questionnaire</a> 
                         @elseif($employee->health_verified == 'approved')
                             Health Questionnaire Approved
                             @else
                             Pending
                        @endif
                            </td>
...
$(document).ready(function(){
  $('#search').keyup(function(){
    ...
    $.ajax({
        type:'get',
        url: '{{ route('search.healthDetails') }}',
        data:{'search':$value},
