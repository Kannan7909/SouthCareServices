     ...
     <table class="table table-striped table-hover table-bordered">
                ...
                <tbody class="allData">
                    ...
                    <th scope="row">{{ $loop->iteration }}</th>
                        ...
                        <td>
                        @if($employee->application_verified == 'processing')
                             <a href="{{ route('approve.application',encrypt($employee->id)) }}" class="edit btn btn-primary" title="Confirm" data-toggle="tooltip"><i class="material-icons">&#xE254;</i>Appove Application Details</a> 
                         @elseif($employee->application_verified == 'approved')
...
$(document).ready(function(){
  $('#search').keyup(function(){
    ...
    $.ajax({
        type:'get',
        url: '{{ route('search.applicationDetails') }}',
        data:{'search':$value},
