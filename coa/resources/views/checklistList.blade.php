...
<head>
        <meta charset="utf-8" />
        <title>Induction Checklist List</title>
                    ...
                    <div class="row">
                            <div class="col-md-5">
                                <div class="page-title-box">                                   
                                    <h4 class="page-title">Induction Checklist Status</h4>
                                </div>
                            </div>
...
<!--                                <option value="registration_completed">All Users</option>
                                  ...
                                  <!--    <option value="checklist_commented">Checklist Commented </option>
                                    <option value="checklist_under Review">Checklist Under Review </option>
                                    <option value="checklist_approved"> Checklist Approved</option>
                                ...
                                <div class="card">
                                    ...
                                    <div id="education-work-details" class="education-work-details">
                                            ...
                                            <thead>   
                                                ...
                                                @else
                                                 @foreach ($checklistData as $employee)
                                                <tr>
                                                <th scope="row">{{ $loop->iteration }}</th>
                                                    ...
                                                    @if($employee->induction_checklist == "Submitted")
                                                    <td><i class="mdi mdi-circle text-primary"></i>Checklist {{ $employee->induction_checklist }}</td>
                                                    @endif
                                                    @if($employee->induction_checklist == "Under Review")
                                                    <td><i class="mdi mdi-circle text-warning"></i>Checklist {{ $employee->induction_checklist }}</td>
                                                    @endif
                                                    @if($employee->induction_checklist == "Approved")
                                                    <td><i class="mdi mdi-circle text-success"></i>Checklist {{ $employee->induction_checklist }}</td>
                                                    @endif
                                                    <td>
                                                        <a href="{{ route('view.checklist',encrypt($employee->employee_id)) }}" class="btn btn-primary" title="View" data-toggle="tooltip">View</a>
                                                  </td>
                                                </tr> 
                                                @endforeach
                                                @endif
    ...
    //console.log(data[0]);
    var column = "induction_checklist";
    var status = data[1];
if(value){
        $('.allData').hide();
        $('.searchData').show();
