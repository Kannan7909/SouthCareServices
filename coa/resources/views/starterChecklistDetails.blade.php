...
<head>
    ...
    <title>Start Checklist Details</title>
    ...
    <table class="table table-striped table-hover table-bordered">
                ...
                <tbody class="allData">
                    ...
                    <th scope="row">{{ $loop->iteration }}</th>
                        ...
                        <td>
                        @if($employee->starter_verified == 'processing')
                             <a href="{{ route('approve.starter',encrypt($employee->id)) }}" class="edit btn btn-primary" title="Confirm" data-toggle="tooltip"><i class="material-icons">&#xE254;</i>Appove Starter Checklist</a> 
                         @elseif($employee->starter_verified == 'approved')
                             Starter Checklist Approved
                             @else
                             Pending
                        @endif
                            </td>
...
$(document).ready(function(){
  $('#search').keyup(function(){
    ...
    $.ajax({
        type:'get',
        url: '{{ route('search.starterDetails') }}',
        data:{'search':$value},
